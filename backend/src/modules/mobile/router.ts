...
mobileRouter.get('/bootstrap', async (req, res, next) => {
  try {
    ...
    const reviewers = await prisma.user.findMany({
      where: { companyId: auth.companyId, isActive: true, roles: { some: { role: { code: 'inspector' }, ...(hasCompanyScope ? {} : { OR: [{ objectId: null }, { objectId: { in: objectIds } }] }) } } },
    ...
    return res.json({ serverTime: new Date().toISOString(), reviewers: reviewers.map((reviewer) => ({ id: reviewer.id, name: reviewer.fullName, objectIds: reviewer.roles.flatMap((role) => role.objectId ? [role.objectId] : []) })), objects: objects.map((object) => {
      ...
      return {
        id: object.id, name: localized(object.name, object.nameUz), address: localized(object.address ?? '', object.addressUz), latitude: object.latitude, longitude: object.longitude, status: object.status,
        ...
        defects: object.defects.map((defect) => ({ id: defect.id, objectId: object.id, taskId: defect.taskId, description: defect.description, status: defect.status, beforePhotos: defect.beforePhotos, afterPhotos: defect.afterPhotos, dueAt: defect.dueAt?.toISOString() ?? null, resolvedAt: defect.resolvedAt?.toISOString() ?? null, createdAt: defect.createdAt.toISOString() })),
